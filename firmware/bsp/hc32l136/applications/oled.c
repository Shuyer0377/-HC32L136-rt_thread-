...
{
	rt_uint8_t i,m,temp,size2,chr1;
	rt_uint8_t x0=x,y0=y;
	if(size1==8)size2=6;
...
void OLED_ShowChinese(rt_uint8_t x,rt_uint8_t y,rt_uint8_t num,rt_uint8_t size1,rt_uint8_t mode)
{
	rt_uint8_t m,temp;
	rt_uint8_t x0=x,y0=y;
	rt_uint16_t i,size3=(size1/8+((size1%8)?1:0))*size1;  //得到字体一个字符对应点阵集所占的字节数
	for(i=0;i<size3;i++)
	{
		if(size1==16)
